...
import com.iprism.ecmoutboundmarketing.databinding.FragmentMyLeadsArogyaSreeBinding
...
class MyLeadsArogyaSreeFragment : Fragment() {

    private lateinit var binding: FragmentMyLeadsArogyaSreeBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentMyLeadsArogyaSreeBinding.inflate(inflater, container, false)
        setupArogyaSreeAdapter()
        return binding.root
    }

    private fun setupArogyaSreeAdapter() {
        var myLeadsCashAdapter = MyLeadCashAdapter(requireContext())
        var linearLayoutManager = LinearLayoutManager(requireContext())
        binding.arogyaSreeRv.adapter = myLeadsCashAdapter
        binding.arogyaSreeRv.layoutManager = linearLayoutManager
